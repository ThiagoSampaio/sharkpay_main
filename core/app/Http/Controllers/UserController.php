...
class UserController extends Controller
{
    ...
    //Store
        ...
        public function storefront()
        {
            ...
            $data['product'] = Product::whereUser_id(Auth::guard('user')->user()->id)->with('images')->orderBy('id', 'desc')->get();
            $data['category']=Productcategory::whereUser_id(Auth::guard('user')->user()->id)->get();
            $data['received']=Order::whereStatus(1)->whereseller_id(Auth::guard('user')->user()->id)->sum('total');
            $data['total']=Order::whereseller_id(Auth::guard('user')->user()->id)->sum('total');  
        ...
        public function submitproduct(Request $request)
        {
            ...
            $request->validate([
                ...
                'category' => 'required|integer|exists:product_category,id',
                'amount' => 'required|numeric|min:0',
                'price_promo' => 'nullable|numeric|min:0',
                'quantity' => 'required|integer|min:1',
                'shipping_status' => 'required|in:0,1',
                'delivery_type' => 'nullable|in:digital,fisico,servico',
                'external_link' => 'nullable|url',
                'file' => 'nullable|image|mimes:jpeg,jpg,png,gif|max:2048'
            ]);

            $trx = 'PRD-' . Str::random(6);
            ...
            $product = Product::create([
                ...
                'amount' => $request->amount,
                'price_promo' => $request->price_promo ?? 0.00,
        ...
        public function storelink($id)
        {
            ...
            if($store->user->status==0){
                ...
                $data['products']=Product::whereUser_id($store->user_id)->orderby('id', 'desc')->get();
                $data['title']=$store->store_name; 
