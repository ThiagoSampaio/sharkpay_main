...
class ProductPaymentController extends Controller {
    ...
    private function payOrderWithPix(Request $request, $product, $currency, $settings) {
        ...
        // Se usuário está autenticado, buscar dados do banco
        if (Auth::guard('user')->check()) {
            $authenticatedUser = Auth::guard('user')->user();
            $document = BillingUtils::getUserDocument($authenticatedUser->id);
            $clientName = $authenticatedUser->first_name . ' ' . $authenticatedUser->last_name;

            if (empty($document)) {
                throw new \Exception("É necessário cadastrar seu documento no perfil antes de realizar pagamentos via PIX.");
            }
        } else {
            // Usuário não autenticado, buscar dados do request
            $document = $request->document ?? null;
            $clientName = $request->first_name ?? null;

            if (empty($document) || empty($clientName)) {
