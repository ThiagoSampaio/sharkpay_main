  ...
  <div class="content-wrapper">
    <div class="panel-page-stack">
      <div class="panel-page-hero">
        <span class="panel-page-hero__eyebrow"><i data-lucide="settings-2" style="width: 14px; height: 14px;"></i> Configuracoes da conta</span>
        <h1 class="panel-page-hero__title">Organize perfil, seguranca, dados bancarios e conformidade em um unico fluxo.</h1>
        <p class="panel-page-hero__copy">Esta area concentra as configuracoes essenciais da sua operacao. Mantenha dados de contato, chaves de API, contas bancarias e compliance atualizados para evitar bloqueios, falhas em repasses e retrabalho operacional.</p>
        <div class="panel-page-hero__meta">
          <div class="panel-page-meta-card">
            <span>Workspace</span>
            <strong>{{ $user->business_name ?: trim($user->first_name.' '.$user->last_name) }}</strong>
          </div>
          <div class="panel-page-meta-card">
            <span>Status da conta</span>
            <strong>{{ $user->kyc_status == 0 ? 'Verificacao pendente' : 'Conta verificada' }}</strong>
          </div>
          <div class="panel-page-meta-card">
            <span>Email de suporte</span>
            <strong>{{ $user->support_email ?: 'Nao configurado' }}</strong>
          </div>
        </div>
      </div>
      ...
      <div class="panel-note">
        ...
        <div>
          <strong>Priorize os blocos mais sensiveis.</strong>
          <p>Se voce opera recebimento, saque ou integracao, revise primeiro Seguranca, Dados bancarios e Compliance. Isso reduz risco de falha em transacoes, bloqueios e divergencias cadastrais.</p>
        </div>
      </div>
      ...
      <div class="col-md-12">
        <div class="nav-wrapper">
          <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
              ...
              <li class="nav-item">
                  <a class="nav-link mb-sm-3 mb-md-0 @if(route('user.security')==url()->current()) active @endif" id="tabs-icons-text-2-tab" data-toggle="tab" href="#tabs-icons-text-2" role="tab" aria-controls="tabs-icons-text-2" aria-selected="false"><i class="fad fa-cogs"></i> {{$lang["profile_index_security"]}}</a>
              </li>
              <li class="nav-item">
                  <a class="nav-link mb-sm-3 mb-md-0 @if(route('user.api')==url()->current()) active @endif" id="tabs-icons-text-3-tab" data-toggle="tab" href="#tabs-icons-text-3" role="tab" aria-controls="tabs-icons-text-3" aria-selected="false"><i class="fad fa-key"></i> {{$lang["profile_index_api_keys"]}}</a>
              </li>        
              <li class="nav-item">
                  <a class="nav-link mb-sm-3 mb-md-0 @if(route('user.social')==url()->current()) active @endif" id="tabs-icons-text-4-tab" data-toggle="tab" href="#tabs-icons-text-4" role="tab" aria-controls="tabs-icons-text-4" aria-selected="false"><i class="fad fa-share-square"></i> {{$lang["profile_index_social_profile"]}}</a>
              </li>                
              <li class="nav-item">
                  <a class="nav-link mb-sm-3 mb-md-0 @if(route('user.bank')==url()->current()) active @endif" id="tabs-icons-text-5-tab" data-toggle="tab" href="#tabs-icons-text-5" role="tab" aria-controls="tabs-icons-text-5" aria-selected="false"><i class="fad fa-university"></i> {{$lang["profile_index_bank_accounts"]}}</a>
              </li>              
              <li class="nav-item">
                  <a class="nav-link mb-sm-3 mb-md-0 @if(route('user.compliance')==url()->current()) active @endif" id="tabs-icons-text-6-tab" data-toggle="tab" href="#tabs-icons-text-6" role="tab" aria-controls="tabs-icons-text-6" aria-selected="false"><i class="fad fa-globe"></i> {{$lang["profile_index_compliance"]}}</a>
              </li>        
          </ul>
        </div>
