        ...
        <div class="card">
          ...
          <div class="card-body">
            <form action="{{route('product.feature.submit')}}" method="post">
              ...
              <div class="form-group row">
                <label class="col-form-label col-lg-5">Preço Promocional</label>
                <div class="col-lg-7">
                  <div class="input-group">
                    <div class="input-group-prepend">
                      <span class="input-group-text">{{$currency->symbol}}</span>
                    </div>
                    <input type="number" step="any" name="price_promo" value="{{$product->price_promo}}" class="form-control" placeholder="Opcional">
                  </div>
                </div>
              </div>

              <div class="form-group row">
                <label class="col-form-label col-lg-5">Tipo de Entrega</label>
                <div class="col-lg-7">
                  <select class="form-control custom-select" name="delivery_type">
                    <option value="">Selecione</option>
                    <option value="digital" @if($product->delivery_type == 'digital') selected @endif>Produto Digital</option>
                    <option value="fisico" @if($product->delivery_type == 'fisico') selected @endif>Produto Físico</option>
                    <option value="servico" @if($product->delivery_type == 'servico') selected @endif>Serviço</option>
                  </select>
                </div>
              </div>

              <div class="form-group row">
                <label class="col-form-label col-lg-5">Link Externo</label>
                <div class="col-lg-7">
                  <input type="url" name="external_link" class="form-control" value="{{$product->external_link}}" placeholder="https://...">
                </div>
              </div>

              <div class="form-group row">
                <label class="col-form-label col-lg-5">Tags</label>
                <div class="col-lg-7">
                  <input type="text" name="tags" class="form-control" value="{{$product->tags}}" placeholder="ex: ebook, receitas, IA">
                </div>
              </div>
