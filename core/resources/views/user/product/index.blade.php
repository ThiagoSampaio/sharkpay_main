  ...
  <div class="content-wrapper">
    <div class="row align-items-center py-4">
      ...
      <div class="col-lg-6 col-5 text-right">
        <a data-toggle="modal" data-target="#category" href="" class="btn btn-sm btn-neutral btn-modern"><i class="fad fa-filter"></i> {{$lang["product_list_category"]}}</a> 
        <a data-toggle="modal" data-target="#new-product" href="" class="btn btn-sm btn-neutral btn-modern"><i class="fad fa-plus"></i> {{$lang["product_list_create_product"]}} 
      </div>
    </div>
          ...
          <div class="modal-body">
            <form action="{{route('submit.category')}}" method="post">
              ...
              <div class="text-right">
                <button type="submit" class="btn btn-neutral btn-modern btn-block">{{$lang["product_list_create_category"]}}</button>
              </div>
                    ...
                    <li class="list-group-item px-0">
                      <div class="row align-items-center">
                        ...
                        <div class="col-4 text-right">
                          <a href="{{route('delete.category', ['id' => $val->id])}}" class="btn btn-sm btn-neutral btn-modern"><i class="fad fa-trash"></i> {{$lang["product_list_delete"]}}</a>
                        </div>
                      </div>
                    </li>
                  @endforeach
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{route('submit.product')}}" method="post" enctype="multipart/form-data">
              ...
              <div class="text-right">
                <button type="submit" class="btn btn-neutral btn-modern btn-primary-modern btn-block">{{$lang["product_list_create_product"]}}</button>
              </div>
            </form>
          </div>
        </div>
    ...
    <div class="row">  
          ...
          <div class="card product-card modern-card">
            ...
            <div class="card-body">
              <div class="product-card-header">
                <div class="product-price">{{$currency->symbol}} {{number_format($val->amount)}}.00</div>
                <div>
                ...
                <div class="">
                  ...
                  <div class="product-stats">
                    <div class="product-stat">
                      <span class="product-stat-label">{{$lang["product_list_sold"]}}</span>
                      <span class="product-stat-value">{{$val->sold}}/{{$val->quantity}}</span>
                    </div>
                  </div>
                  <p class="mb-3"><a class="btn-icon-clipboard" data-clipboard-text="{{route('product.link', ['id' => $val->ref_id])}}" title="{{$lang["product_list_copy"]}}"><i class="fad fa-link"></i> {{$lang["product_list_copy_product_link"]}}</a></p>
                  @if($val->status==1)
                      <span class="badge badge-modern badge-success-modern"><i class="fad fa-check"></i> {{$lang["product_list_active"]}}</span>
                  @else
                      <span class="badge badge-modern badge-danger-modern">{{$lang["product_list_disabled"]}}</span>
                  @endif
                    ...
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            <div class="card bg-white border-0 mb-0">
                                ...
                                <div class="card-body">
                                    <a  href="{{route('delete.product', ['id' => $val->id])}}" class="btn btn-danger btn-modern btn-block">{{$lang["product_list_proceed"]}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div class="col-md-12 mb-5">
        <div class="empty-state">
          ...
          <h3 class="empty-state-title">{{$lang["product_list_no_product_found"]}}</h3>
          <p class="empty-state-text">{{$lang["product_list_we_couldnt_find_any_product"]}}</p>
        </div>
      </div>
      @endif
      </div>
          ...
          <div class="card-body">
            <div class="row align-items-center">
              <div class="col text-center">
                <h4 class="mb-4 text-white font-weight-bolder">
                {{$lang["product_list_statistics"]}}
                </h4>
                <span class="text-sm text-white mb-0"><i class="fa fa-google-wallet"></i> {{$lang["product_list_received"]}}</span><br>
                <span class="text-xl text-white mb-0">{{$currency->name}} {{number_format($received)}}.00</span><br>
            ...
            <div class="stats-row">
              <div class="">
                <div class="">
                  <span class="stat-label text-white">{{$lang["product_list_pending"]}}</span><br>
                  <span class="stat-label text-white">{{$lang["product_list_total"]}}</span>
                </div>
              </div>
              <div class="">
                <div class="">
                  <span class="stat-value text-white">{{$currency->name}} 00.00</span><br>
                  <span class="stat-value text-white">{{$currency->name}} {{number_format($total)}}.00</span>
                </div>
              </div>
            </div>
          </div>
