...
<div class="container-fluid mt--6">
  <div class="content-wrapper">
    @php
      $activeCards = count($card->where('status', 1));
      $blockedCards = count($card->where('status', 2));
    @endphp
    <div class="panel-page-stack">
      <div class="panel-page-hero">
        <span class="panel-page-hero__eyebrow"><i data-lucide="credit-card" style="width: 14px; height: 14px;"></i> Operacao de cartoes virtuais</span>
        <h1 class="panel-page-hero__title">Crie, acompanhe e movimente cartoes virtuais com mais clareza operacional.</h1>
        <p class="panel-page-hero__copy">Use este painel para emitir novos cartoes, acompanhar saldo em tempo real, congelar quando necessario e registrar funding ou retirada de forma mais segura. A criacao considera limites e encargos definidos para sua conta.</p>
        <div class="panel-page-hero__actions">
          <a data-toggle="modal" data-target="#modal-formx" href="" class="btn btn-neutral"><i class="fa fa-plus"></i> {{$lang["virtual_create_card"]}}</a>
        </div>
        <div class="panel-page-hero__meta">
          <div class="panel-page-meta-card">
            <span>Cartoes emitidos</span>
            <strong>{{ count($card) }}</strong>
          </div>
          <div class="panel-page-meta-card">
      ...
      <div class="panel-note">
        ...
        <div>
          <strong>Como usar este modulo.</strong>
          <p>Crie cartoes apenas para operacoes que realmente exijam emissao. Antes de funding, revise saldo maximo permitido e o custo de criacao ou movimentacao para evitar consumo desnecessario de margem.</p>
        </div>
      </div>
          ...
          <div class="modal-body p-0">
            <div class="card bg-white border-0 mb-0">
              <div class="card-header">
                ...
                <p class="form-text text-xs">{{$lang["virtual_card_creation_charge_is"]}} {{$set->virtual_createcharge}}% + {{$currency->symbol.$set->virtual_createchargep}}. Defina o nome do titular, valor inicial e estilo do cartao. O limite operacional atual vai de {{$currency->symbol.number_format($set->vc_min)}} ate {{$currency->symbol.number_format($set->vc_max)}}.</p>
              </div>
              <div class="card-body">
